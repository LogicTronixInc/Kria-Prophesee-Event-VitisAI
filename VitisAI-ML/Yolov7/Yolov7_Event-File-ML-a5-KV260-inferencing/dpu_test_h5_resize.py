...
def runYolo(dpu_runner_tfYolo, image, ori_frame):
    ...
    #cv2.rectangle(ori_frame,(10,10),(100,100),(255,0,0),2)
    #cv2.imshow("newone",ori_frame)
    ...
    for idx, detections in enumerate(batch_detections):
        if detections is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                ...
                y1 = (y1 / pre_h) * ori_h
                x1 = (x1 / pre_w) * ori_w
                #cv2.imshow("beofre_rect",ori_frame)
                # Create a Rectangle patch
                cv2.rectangle(ori_frame, (int(x1), int(y1)), (int(x1 + box_w), int(y1 + box_h)), color, 2)
                #cv2.imshow("after_rect",ori_frame)

                #cv2.waitKey(0)
                # Add label
                label = classes[int(cls_pred)]
                #cv2.putText(ori_frame, str(conf), (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        ...
        # Display image
        #cv2.imwrite("output_frame_resize/10.jpg",ori_frame)
...
def letterbox_resize_opencv(original_image, target_size=(640,640), fill_color=(255, 255, 255)):
    ...
    canvas = cv2.resize(original_image, (640,640))
...
def main(argv):
    ...
    h5_file_name = argv[2]
    events_file = h5py.File(h5_file_name, "r+")
    
    for idx in range(0,len(events_file['data'])):

         img_h5 = np.transpose(events_file['data'][idx], (1, 2, 0))
         stride = 32
         imgsz = 640
        #  img = letterbox_resize_opencv(loaded_array).astype(float)
         img = resize2SquareKeepingAspectRation(img_h5, 640, cv2.INTER_AREA).astype(float)
         # img = letterbox_new(loaded_array, imgsz, stride=stride)[0].transpose(2, 0, 1)
        
         im0 = viz_histo_binarized_new(img.copy().transpose(2, 0, 1))
         im1 = cv2.cvtColor(im0, cv2.COLOR_RGB2BGR)
         #cv2.imshow("im0",im1)
         #cv2.waitKey(0)
         height, width, _ = im0.shape
         #img /= 255.0 
         runYolo(dpu_runners, img, im1)
         #del dpu_runners
