 ...
 * @return none
 */
void runYOLO(vart::Runner* runner,float conf,float rmax, float bmax,cv::Mat* img) {
  /* mean values for YOLO-v3 */
  float mean[3] = {0.0001566, 0.00001234, 0.3f};
  ...
  std::vector<vart::TensorBuffer*> inputsPtr, outputsPtr;
    ...
    for (int h = 0; h < height; h++) {
      for (int w = 0; w < width; w++) {
        for (int c = 0; c < 1; c++) {
            image3.at<Vec3b>(h, w)[c] = map_to_0_1_with_step_size(int(image2.at<Vec3b>(h, w)[c]/17));        
        }
      }
    }
    ...
    for (int h = 0; h < height; h++) {
      for (int w = 0; w < width; w++) {
        for (int c = 2; c < 3; c++) {
            image3.at<Vec3b>(h, w)[c-1] =  map_to_0_1_with_step_size(int(image2.at<Vec3b>(h, w)[c]/17));
        }
      }
    }
