...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            self.img_files = sorted(x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() == "h5")
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
            self.labels = [x.rsplit('.', 1)[0] + '_bbox.npy' for x in self.img_files]
            # print(img_files)
            all_labels = []
            my_list = []
            segments = []
            ...
            for file_idx in range(len(self.img_files)):
                print("cache h5 file is ",self.img_files[file_idx])
                print("\n\n\n")
                with h5py.File(self.img_files[file_idx], "r") as h5_file:
                    num_frames, _, height, width = h5_file["data"].shape
                # print("num frames", num_frames)
                labels = np.load(self.labels[file_idx])
                ...
                shape_event =(height, width)     
                im_file_event_ =("{}").format(self.img_files[file_idx])
    
                for frame_idx in range(num_frames):
                    try:
    
                        im_file_event=("{}@{}").format(im_file_event_,frame_idx)
                        nf += 1
    
                        
                        box_idxs = np.nonzero(timestamps == my_list[frame_idx])[0]
    
                        # print("frame idx box_idxs  ",frame_idx,box_idxs)
                        if len(box_idxs) > 0:
                            
    
                            bboxes = bboxes_normalized[box_idxs, :]
                            classes_list = classes[box_idxs, np.newaxis]
                            l = []
                            for i in range(0,len(bboxes)):
                                value=[]
                                # print(bboxes_normalized[box_idxs, :][i])
                                # print("\n")
                                # print(classes_list[i])  # n, 4)
                                # print()
                                # value = [("{}{}{}{}{}").format(classes_list[i][0],bboxes_normalized[box_idxs, :][i][0],bboxes_normalized[box_idxs, :][i][1],bboxes_normalized[box_idxs, :][i][2],bboxes_normalized[box_idxs, :][i][3])]
                                value.append(classes_list[i][0])
                                value.append(bboxes_normalized[box_idxs, :][i][0])
                                value.append(bboxes_normalized[box_idxs, :][i][1])
                                value.append(bboxes_normalized[box_idxs, :][i][2])
                                value.append(bboxes_normalized[box_idxs, :][i][3])
                                # print(type(value))
                                l.append(value)
                            ...
                            y[im_file_event] = [l, shape_event, segments] 
                            # print("y im file in event",y[im_file_event]) 
                    except Exception as e: 
                        nc += 1
            ...
            if nf == 0:
                ...
                print("no label foundsss")
    
            y['hash'] = get_hash(self.img_files)
            y['results'] = nf, nm, ne, nc, i + 1
            y['version'] = 0.1  # cache version 
    
            return y   
    
    
    
    def cache_labels_new(self, path=Path('./labels.cache'), prefix=''):
        # Cache dataset labels, check images and read shapes
        x = {}  # dict
        nm, nf, ne, nc = 0, 0, 0, 0  # number missing, found, empty, duplicate
        pbar = tqdm(zip(self.img_files, self.labels), desc='Scanning images', total=len(self.img_files))
        ...
        x['hash'] = get_hash(self.labels + self.img_files)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # Load image
            img, (h0, w0), (h, w), im_file = load_image(self, index)
...
def load_image(self, i, rect_mode=True):
    h5_file = str(self.img_files[i].split("@")[0])
    # h5_file = "data/dataset_precomputed/train/moorea_2019-01-30_000_td_500000_10500000.h5"
    frame_idx = int(self.img_files[i].split("@")[1])
    ev_frame_identifier =  h5_file + "_frame_" + str(frame_idx)
    ...
    elif not (h0 == w0 == self.img_size):  # resize by stretching image to square imgsz
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w), im_file = load_image(self, index)
