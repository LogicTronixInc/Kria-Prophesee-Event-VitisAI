...
//  vector<float> biases{30,61,62,45,59,119,
 ...
 auto conf_desigmoid = -logf(1.0f / conf - 1.0f);
  ...
  for (int h = 0; h < height; ++h) {
	...
	//if (obj_score < CONF) continue;
       ...
       // std::cout<<"2 * c + 2 *anchorCnt * num "<<2 * c + 2 *anchorCnt * num<<std::endl;
       // std::cout<<"2 * c + 2 *anchorCnt * num + 1 "<<2 * c + 2 *anchorCnt * num + 1<<std::endl;
