...
PeriodicHistoFrameGenerationAlgorithm::PeriodicHistoFrameGenerationAlgorithm(int sensor_width, int sensor_height,
    ...
    force_next_frame_(false) {
        ...
        set_fps(fps);
        handle = pp_kernel_init(1280, 720, 1);
        reset();
    }
...
void PeriodicHistoFrameGenerationAlgorithm::process_new_slice(EventBufferReslicerAlgorithm::ConditionStatus slicing_status,
    ...
    auto now_pns = std::chrono::system_clock::now();

    // Convert the current time to time since epoch
    auto duration_pns = now_pns.time_since_epoch();

    // Convert duration to milliseconds
    auto milliseconds_pns
    ...
    int8_t cd_frame_buffer[1280*720*3];
    ...
    // running cv histo frame accel
    cd_frame_accel(handle, (unsigned char *)&histo[0], 720, 2560, 1280, 720, cd_frame_buffer, 1);
    
    memcpy(visu_histo.data, &cd_frame_buffer[0], 1280*720*3);
