...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        cache_path = (p if p.is_file() else Path(self.labels[0]).parent).with_suffix('.cache')  # cached labels
        print("path is : \n\n",cache_path)
...
def load_image(self, i, rect_mode=True):
    ...
    ev_frame_identifier = self.img_files[file_idx] + "_frame_" + str(frame_idx)
    with h5py.File(self.img_files[file_idx], "r") as h5_file:
